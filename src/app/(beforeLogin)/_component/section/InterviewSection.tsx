...
export default function InterviewSection() {
  ...
  const slides = [<MockInterviewTopic key="a" />, <MockChat key="b"/>];
  ...
  const leftColor = useTransform(
    t, // inputValue
    [0, 1], // inputRange
    ["rgba(255,255,255,0.9)", "#FFEDD5"] // outputRange
  );
            ...
            <div className="max-w-2xl mx-auto flex flex-col items-center text-center pt-8 md:pl-8 lg:pl-16 md:pt-16 lg:pt-24 md:mx-0 md:items-start md:text-left">
              <div className="flex flex-col gap-1 w-full">
                <div className="flex justify-center md:justify-start md:flex-col gap-3">
                  <div className="w-8 h-8 md:w-32 md:h-32 lg:w-36 lg:h-36 rounded-full bg-orange-100 border-2 md:border-4 border-orange-500 flex items-center justify-center md:mb-5">
                    <MessageSquare className="w-4 h-4 md:w-16 md:h-16 lg:w-20 lg:h-20 text-orange-500" />
                  </div>
                  ...
                  <h2 className="font-extrabold tracking-tight text-lg sm:text-5xl md:text-6xl lg:text-7xl text-left">
                    ...
                    <span className="hidden md:block">
                      <span className="block md:mb-4">AI와 실전 같은</span>
                      <span className="block">면접 연습</span>
                    </span>
