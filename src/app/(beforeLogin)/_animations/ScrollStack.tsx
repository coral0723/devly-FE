...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const handleScroll = useCallback(() => {
    ...
    // Check if ScrollStack has reached the end
    const scroller = scrollerRef.current;
    if (scroller) {
      ...
      if (isAtEnd && !isAtStart) {
        isScrollStackActiveRef.current = false;
      } else if (isAtStart) {
        isScrollStackActiveRef.current = true;
      }
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    // If scrolling up and ScrollStack is not active, check if we should reactivate it
    if (scrollingUp && !isScrollStackActiveRef.current) {
      // Check if page is at top or near top
      const pageScrollTop = window.pageYOffset || document.documentElement.scrollTop;
      if (pageScrollTop <= 50) { // Small threshold for smoother transition
        isScrollStackActiveRef.current = true;
        document.body.style.overflow = 'hidden';
        scroller.style.pointerEvents = 'auto';
        
        // Set ScrollStack to bottom position for smooth transition
        scroller.scrollTop = scroller.scrollHeight - scroller.clientHeight;
        e.preventDefault();
        return;
      }
      // If page is not at top, allow page scroll
      return;
    }
    
    // If ScrollStack is active, handle scroll within it
    if (isScrollStackActiveRef.current) {
      document.body.style.overflow = 'hidden';
      scroller.style.pointerEvents = 'auto';
      
      // If at start and scrolling up, allow page scroll
      ...
      // Otherwise prevent default and let Lenis handle it
      e.preventDefault();
    }
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    // Initially block parent page scrolling and activate ScrollStack
    document.body.style.overflow = 'hidden';
    isScrollStackActiveRef.current = true;
    
    // Add wheel event listener for better control
    ...
    return () => {
      // Restore parent page scrolling on cleanup
      document.body.style.overflow = 'auto';
